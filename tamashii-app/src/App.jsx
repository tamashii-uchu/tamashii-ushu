...
const TAMASHII_SOGO={0:"鏡の関係",3:"炎の関係",6:"輝きの関係",9:"防護の関係",12:"成長の関係",15:"善悪の関係",18:"和みの関係",21:"発想の関係",24:"無敵の関係",27:"医芸の関係",30:"守護の関係",33:"薬毒の関係",36:"光の関係",39:"伝導の関係",42:"変化の関係",45:"浄化の関係",48:"結束の関係",51:"魅惑の関係",54:"癒しの関係",57:"磨きの関係",60:"磁石の関係"};
...
export default function App(){
  ...
  return(<div style={{minHeight:"100vh",background:`linear-gradient(160deg,#faf6ee,#f5edd8,#faf6ee)`,color:C.text,fontFamily:"'Georgia','Yu Mincho','Noto Serif JP',serif"}}>
    ...
    <div style={{maxWidth:"640px",margin:"0 auto",padding:"0 24px 80px"}}>
      {activeTab==="kojin"&&<div><div style={{padding:"32px 0 24px"}}><InputForm onSubmit={handleKojin} buttonLabel="鑑 定 す る"/></div>{kojinResult&&<div style={{opacity:kojinAnim?1:0,transform:kojinAnim?"translateY(0)":"translateY(20px)",transition:"all 0.5s ease"}}><KojinResult result={kojinResult}/><div style={{textAlign:"center",marginTop:"40px"}}><button onClick={()=>window.print()} style={{padding:"14px 32px",background:"linear-gradient(135deg,#c8a030,#e0c060)",border:`1px solid ${C.gold}`,borderRadius:"4px",color:"#2a2010",fontSize:"13px",letterSpacing:"0.3em",cursor:"pointer",fontFamily:"inherit"}}>✦ 鑑定書を印刷する</button></div></div>}</div>}
      {activeTab==="aisho"&&<div>
        {aishoStep===1&&<div style={{padding:"32px 0 24px"}}><div style={{textAlign:"center",marginBottom:"24px"}}><div style={{fontSize:"10px",color:C.gold,letterSpacing:"0.3em"}}>STEP 1 / 2</div><div style={{fontSize:"16px",color:C.text,marginTop:"8px"}}>Aさんの情報を入力</div></div><InputForm onSubmit={handleAishoA} buttonLabel="次 へ →"/></div>}
        {aishoStep===2&&<div style={{padding:"32px 0 24px"}}><div style={{textAlign:"center",marginBottom:"24px"}}><div style={{fontSize:"10px",color:C.gold,letterSpacing:"0.3em"}}>STEP 2 / 2</div><div style={{fontSize:"16px",color:C.text,marginTop:"8px"}}>Bさんの情報を入力</div><div style={{marginTop:"8px",padding:"8px 16px",background:"rgba(200,168,74,0.1)",border:`1px solid ${C.borderGold}`,borderRadius:"4px",fontSize:"12px",color:C.muted}}>✓ {resultA?.name} さんの情報を受け取りました</div></div><InputForm onSubmit={handleAishoB} buttonLabel="相 性 を 診 断"/><button onClick={resetAisho} style={{width:"100%",marginTop:"12px",padding:"12px",background:"transparent",border:`1px solid ${C.borderGold}`,borderRadius:"4px",color:C.muted,fontSize:"12px",cursor:"pointer",fontFamily:"inherit"}}>← 最初からやり直す</button></div>}
